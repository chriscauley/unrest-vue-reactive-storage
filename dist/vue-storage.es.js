...
const newThread = (f) => new Promise((resolve) => {
...
var MemoryStorage = (slug, options = {}) => {
  const {
    makePaginator = defaults$4.makePaginator,
    prepareItem = defaults$4.prepareItem,
    initial = {},
    afterSave = () => {
    },
    fromServer = (d) => d,
  ...
  return {
    ...
    save: (data2) => newThread(() => {
      data2 = toServer(prepareItem(data2, () => ID_COUNTER++));
      state[data2.id] = data2;
      afterSave(state);
      return data2;
    }),
    delete: ({ id }) => newThread(() => {
...
localStorageJson.get = function(key) {
  if (localStorage) {
    try {
...
localStorageJson.set = function(key, value) {
  if (localStorage) {
    try {
...
localStorageJson.remove = function(key) {
  if (localStorage) {
    try {
...
localStorageJson.clear = function() {
  if (localStorage) {
    try {
      localStorage.clear();
      return true;
...
const ReactiveLocalStorage = (options = {}) => {
...
const ReactiveRestApi = (options = {}) => {
  ...
  const fetch = (url) => {
    ...
    if (state.loading[url]) {
      ...
      return client.get(url).then((data2) => {
        var _a;
        ...
        (_a = pending[url]) == null ? void 0 : _a.forEach(([_, reject]) => reject(error));
        throw error;
      });
    }
...
var RestStorage = (slug, options = {}) => {
  ...
  const bounceSave = (data2) => {
    if (!_bouncers[data2.id]) {
      _bouncers[data2.id] = lodash_debounce((data3) => save(data3), 1e3);
    }
    _bouncers[data2.id](data2);
  };
  return {
    api,
    save,
...
export { LocalStorage, MemoryStorage, ReactiveLocalStorage, ReactiveRestApi, RestStorage, index as default, getClient };
